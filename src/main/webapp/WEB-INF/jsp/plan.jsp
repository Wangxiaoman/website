...
<body>
    ...
    <div class="container theme-showcase">
	    ...
	    <div class="jumbotron">
	       <h1 onclick="backPlan();">我的计划列表</a></h1>
	     </div>
    	...
    	<div class="page-header">
					...
					<tbody>
						<c:forEach items="${plans}" var="obj">
							<tr>
									...
									<td style="vertical-align: middle; text-align: center;"><fmt:formatDate value="${obj.createTime}" pattern="yyyy-MM-dd HH"/></td>
								...
								<td>
									<c:if test="${obj.status == 0}">
										<a class="op" href="/plan/status?id=${obj.id}&status=1&parentId=${obj.parentId}" style="color: #2ac1f2">开始</a>
										<a class="op" href="/plan/status?id=${obj.id}&status=2&parentId=${obj.parentId}" style="color: #2ac1f2">完成</a>
									</c:if>
									<c:if test="${obj.status == 1}">
										<a class="op" href="/web/plan/status?id=${obj.id}&status=2&parentId=${obj.parentId}" style="color: #2ac1f2">完成</a>
									</c:if>
									<c:if test="${parentId == 0}">
										<a class="op" href="/plan/toSave?parentId=${obj.id}" style="color: #2ac1f2">新增子计划</a>
										<a class="op" href="/plan/list?parentId=${obj.id}" style="color: #2ac1f2">查看子计划</a>
									</c:if>
								</td>
							</tr>
						</c:forEach>
			...
			<c:if test="${parentId == 0}">
		        <button type="button" class="btn btn-default btn-lg" onclick="toSave('${parentId}')">
				  <span class="glyphicon glyphicon-star" aria-hidden="true"></span> 创建计划
				</button>
			</c:if>
	    </div>
